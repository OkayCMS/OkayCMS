<div class="fn_delivery_novaposhta" {if $delivery->module_id != $novaposhta_module_id} style="display: none;" {/if}>
    ...
    <div class="fn_np_door_delivery_block" {if !$isDoorDelivery} style="display: none" {/if}>
        <div class="mb-1">
            <div class="heading_label">{$btr->order_np_city}</div>
            <input type="text" name="novaposhta_city" class="fn_newpost_city_name form-control" autocomplete="off"
                value="{$novaposhta_delivery_data->city_name|escape}" {if !$isDoorDelivery} disabled{/if}>
        </div>
        <div class="mb-1">
            <div class="heading_label">{$btr->order_np_street}</div>
            <input type="text" name="novaposhta_street" class="fn_newpost_street form-control" autocomplete="off"
                value="{$novaposhta_delivery_data->street|escape}" {if !$isDoorDelivery} disabled{/if}>
        </div>
        <div class="mb-1">
            <div class="heading_label">{$btr->order_np_house}</div>
            <input type="text" name="novaposhta_house" class="form-control" autocomplete="off"
                value="{$novaposhta_delivery_data->house|escape}" {if !$isDoorDelivery} disabled{/if}>
        </div>
        <div class="mb-1">
            <div class="heading_label">{$btr->order_np_apartment}</div>
            <input type="text" name="novaposhta_apartment" class="form-control" autocomplete="off"
                value="{$novaposhta_delivery_data->apartment|escape}" {if !$isDoorDelivery} disabled{/if}>
        </div>

        <input name="novaposhta_city_name" class="fn_np_clear" type="hidden"
            value="{$novaposhta_delivery_data->city_name|escape}" {if !$isDoorDelivery} disabled{/if} />
        <input name="novaposhta_area_name" class="fn_np_clear" type="hidden"
            value="{$novaposhta_delivery_data->area_name|escape}" {if !$isDoorDelivery} disabled{/if} />
        <input name="novaposhta_region_name" class="fn_np_clear" type="hidden"
            value="{$novaposhta_delivery_data->region_name|escape}" {if !$isDoorDelivery} disabled{/if} />
        <input name="novaposhta_street_name" class="fn_np_clear" type="hidden"
            value="{$novaposhta_delivery_data->street|escape}" {if !$isDoorDelivery} disabled{/if} />

        <input name="novaposhta_door_delivery" type="hidden" value="1" {if !$isDoorDelivery} disabled{/if} />
    </div>
    <div class="fn_np_warehouse_delivery_block" {if $isDoorDelivery} style="display: none" {/if}>
        <input name="novaposhta_warehouse_id" type="hidden" value="{$novaposhta_delivery_data->warehouse_id|escape}"
            {if $isDoorDelivery} disabled{/if} />

        <div class="mb-1">
            <div class="heading_label">{$btr->order_np_city}</div>
            <input type="text" class="fn_newpost_city_name form-control" autocomplete="off"
                value="{$novaposhta_delivery_data->city_id|newpost_city}" {if $isDoorDelivery} disabled{/if}>
        </div>
        <div class="mb-1">
            ...
            <select name="novaposhta_warehouse" tabindex="1" class="selectpicker form-control warehouses_novaposhta"
                data-live-search="true" {if $isDoorDelivery} disabled{/if}></select>
        </div>
    </div>

    <div class="mb-1">
        <div class="heading_label">
            <input type="checkbox" id="novaposhta_redelivery" name="novaposhta_redelivery" value="1"
                {if $novaposhta_delivery_data->redelivery}checked{/if} />
    ...
    {else}
        <div class="mb-1">
            <div class="heading_label">
                <span class="fn_np_term" {if !$novaposhta_delivery_data->delivery_term} style="display: none;"
                    {/if}>{$btr->order_np_term}: <span>{$novaposhta_delivery_data->delivery_term|escape}</span></span>
    ...
    <script>
        ...
        $(".fn_newpost_city_name").devbridgeAutocomplete({
            ...
            formatResult: function(suggestion, currentValue) {
                ...
                return "<div style='text-align: left'>" + suggestion.value.replace(new RegExp(pattern, 'gi'),
                    '<strong>$1<\/strong>') + "<\/div>";
            }
        });
        ...
        function setStreetAutocomplete(cityRef) {
            $(".fn_newpost_street").devbridgeAutocomplete({
                ...
                formatResult: function(suggestion, currentValue) {
                    var reEscape = new RegExp( '(\\' + ['/', '.', '*', '+', '?', '|', '(', ')', '[', ']', '{', '}', '\\'].join( '|\\' ) + ')', 'g' );
                    var pattern = '(' + currentValue.replace(reEscape, '\\$1') + ')';
                    return "<div style='text-align: left'>" + suggestion.value.replace(new RegExp(pattern,
                        'gi'), '<strong>$1<\/strong>') + "<\/div>";
                }
            });
        }
        ...
        $(".fn_newpost_city_name").devbridgeAutocomplete({
            ...
            noCache: true,
            onSelect: function(suggestion) {
                $('input[name="novaposhta_warehouse_id"]').val(
                ''); //  очищаем выбранное отделение другого города
            ...
            formatResult: function(suggestion, currentValue) {
                var reEscape = new RegExp( '(\\' + ['/', '.', '*', '+', '?', '|', '(', ')', '[', ']', '{', '}', '\\'].join( '|\\' ) + ')', 'g' );
                var pattern = '(' + currentValue.replace(reEscape, '\\$1') + ')';
                return "<span>" + suggestion.value.replace(new RegExp(pattern, 'gi'), '<strong>$1<\/strong>') +
                    "<\/span>";
            }
        });
