...
/**
 * @phpstan-type PurchaseRow object{
 *     order_id: int|string,
...
class ExportOrders extends AbstractExport
{
    public function export()
    {
        ...
        foreach ($orders as $order) {
            ...
            $doc->addChild("Ид", (string)$order->id);
            ...
            $doc->addChild("Курс", "1");
            $doc->addChild("Сумма", (string)$order->total_price);
            ...
            if ($order->total_price != $startingPrice) {
                ...
                $t1_4 = $t1_3->addChild("Сумма", (string)($startingPrice - $order->total_price));
...
//            $addrField->addChild ( 'Значение', $order->address );
            ...
            if (isset($purchases[$order->id])) {
                foreach ($purchases[$order->id] as $purchase) {
                    if (!empty($purchase->product_id) && !empty($purchase->variant_id)) {
                        ...
                        $productExternalId = $this->integration1C->db->result('external_id');

                        $select = $this->integration1C->queryFactory->newSelect();
                        $select->cols(['external_id'])->from(VariantsEntity::getTable())->where('id=:id')->bindValue('id', $purchase->variant_id);

                        $this->integration1C->db->query($select);
                        $variantExternalId = $this->integration1C->db->result('external_id');

                        // Если нет внешнего ключа товара - указываем наш id
                        if (is_scalar($productExternalId) && $productExternalId !== '') {
                            $id = (string)$productExternalId;
                        } else {
                            ...
                            $id = (string)$purchase->product_id;
                        }

                        // Если нет внешнего ключа варианта - указываем наш id
                        if (is_scalar($variantExternalId) && $variantExternalId !== '') {
                            $id = $id . '#' . (string)$variantExternalId;
                        } else {
                            $update = $this->integration1C->queryFactory->newUpdate();
                        ...
                        $t1_2 = $t1_1->addChild("ЦенаЗаЕдиницу", (string)$purchase->price);
                        $t1_2 = $t1_1->addChild("Количество", (string)$purchase->amount);
                        $t1_2 = $t1_1->addChild("Сумма", (string)($purchase->price * $purchase->amount));
                        ...
                        if ($this->integration1C->exportPurchasesDiscountsSeparate && $purchase->price != $purchase->undiscounted_price) {
                            ...
                            $t1_3 = $t1_2->addChild("Скидка");
                            $t1_4 = $t1_3->addChild("Сумма", (string)($purchase->undiscounted_price - $purchase->price));
                            $t1_4 = $t1_3->addChild("УчтеноВСумме", "true");
                        }
            ...
            if ($order->delivery_price > 0 && !$order->separate_delivery) {
                ...
                $t1->addChild("ЦенаЗаЕдиницу", (string)$order->delivery_price);
                $t1->addChild("Количество", '1');
                $t1->addChild("Сумма", (string)$order->delivery_price);
