...
if (isset($_GET['file']) && !checkRelativePath($_GET['file'])) {
    ...
    $_GET['file'] = normalizeFilemanagerRelativePath($_GET['file']);
}
...
if (isset($_POST['path']) && !checkRelativePath($_POST['path'])) {
    ...
    $_POST['path'] = normalizeFilemanagerRelativePath($_POST['path']);
}
    ...
    switch ($_GET['action']) {
        ...
        case 'save_img':
            ...
            if (!check_extension($info['extension'] ?? '', $config)) {
                ...
                exit(trans('wrong extension') . AddErrorLocation());
            }
            ...
            if ($ftp) {
                $temp = tempnam('/tmp', 'RF');
                unlink($temp);
                $urlExtension = strrchr($_POST['url'], '.');
                $temp .= "." . ($urlExtension === false ? '' : substr($urlExtension, 1));
        ...
        case 'extract':
            ...
            if ($ftp) {
                ...
                $baseRelativePath = fix_dirname($_POST['path']);
                $baseRelativePath = $baseRelativePath === '.' ? '' : $baseRelativePath . '/';
                $path = resolveFilemanagerPath($config['current_path'], $_POST['path']);
                $base_folder = resolveFilemanagerPath($config['current_path'], $baseRelativePath);
                if ($path === null || $base_folder === null) {
                    http_response_code(400);
                    exit(trans('wrong path') . AddErrorLocation());
                }
                $base_folder = rtrim($base_folder, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;
            }
            ...
            if ($ftp) {
                ...
                unlink($temp);
                $temp .= isset($info['extension']) ? "." . $info['extension'] : '';
                $handle = fopen($temp, "w");
                $archiveContent = file_get_contents($path);
                if ($handle === false || $archiveContent === false) {
                    exit;
                }
                ...
                case "zip":
                    ...
                    if ($zip->open($path) === true) {
                        ...
                        for ($i = 0; $i < $zip->numFiles; $i++) {
                            ...
                            if ($aStat === false) {
                                $sizeTotalFinal = $config['MaxSizeTotal'] + 1;
                                break;
                            }
        ...
        case 'media_preview':
            ...
            if ($ftp) {
                ...
                $preview_file = resolveFilemanagerPath($config['current_path'], $_GET["file"]);
            ...
            $previewExtension = strtolower($info['extension'] ?? '');
            ob_start();
            ?>
            ...
            <?php if (in_array($previewExtension, $config['ext_music'])) : ?>
                    ...
                    $("#jquery_jplayer_1").jPlayer({
                        ready: function () {
                            $(this).jPlayer("setMedia", {
                                title: "<?php echo htmlentities((string)($_GET['title'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>",
                                mp3: "<?php echo htmlentities((string)$preview_file, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>",
                                m4a: "<?php echo htmlentities((string)$preview_file, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>",
                                oga: "<?php echo htmlentities((string)$preview_file, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>",
                                wav: "<?php echo htmlentities((string)$preview_file, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>"
                            });
                        },
                        swfPath: "js",
                        solution: "html,flash",
            ...
            <?php elseif (in_array($previewExtension, $config['ext_video'])) :  ?>
            <script type="text/javascript">
                $(document).ready(function () {
                    ...
                    $("#jquery_jplayer_1").jPlayer({
                        ready: function () {
                            $(this).jPlayer("setMedia", {
                                title: "<?php echo htmlentities((string)($_GET['title'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>",
                                m4v: "<?php echo htmlentities((string)$preview_file, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>",
                                ogv: "<?php echo htmlentities((string)$preview_file, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>",
                                flv: "<?php echo htmlentities((string)$preview_file, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>"
                            });
                        },
                        swfPath: "js",
                        solution: "html,flash",
        ...
        case 'cad_preview':
            if ($ftp) {
                ...
                $selected_file = resolveFilemanagerPath($config['current_path'], $_GET['file']);
            ...
            $cad_html = "<iframe src=\"//sharecad.org/cadframe/load?url=" . rfm_attr($cad_url) . "\" class=\"google-iframe\" scrolling=\"no\"></iframe>";
            $ret = $cad_html;
            response($ret)->send();
            break;
        case 'get_file': // preview or edit
            ...
            if ($ftp) {
                ...
                $selected_file = resolveFilemanagerPath(
                    $config['current_path'],
                    $sub_action == 'preview' ? $_GET['file'] : $_POST['path']
                );
                if ($selected_file === null) {
                    http_response_code(400);
                    exit(trans('wrong path') . AddErrorLocation());
            ...
            if ($sub_action == 'preview') {
                if ($preview_mode == 'text') {
                    ...
                    $ret = "<iframe src=\"https://docs.google.com/viewer?url=" . rfm_attr($googledoc_url) . "&embedded=true\" class=\"google-iframe\"></iframe>";
                }
            } else {
                $data = file_get_contents($selected_file);
                if ($data === false) {
