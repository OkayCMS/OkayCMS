...
class UploadHandler
{
    ...
    {
        $this->response = array();
        $redirectTarget = (string)$this->get_server_var('HTTP_REFERER');
        if ($redirectTarget === '') {
            $redirectTarget = $this->get_full_url() . '/';
        }
        $redirectParts = parse_url($redirectTarget);
        $redirectBaseUrl = ($redirectParts['scheme'] ?? '') . '://' . ($redirectParts['host'] ?? '') . '/';
        $this->options = array(
            ...
            'redirect_allow_target' => '/^' . preg_quote(
                $redirectBaseUrl, // Trailing slash to not match subdomains by mistake
    ...
    protected function get_full_url()
    {
        ...
        return ($https ? 'https://' : 'http://') .
            ...
            substr($_SERVER['SCRIPT_NAME'], 0, strrpos($_SERVER['SCRIPT_NAME'], '/') ?: 0);
    }
    ...
    protected function get_file_objects($iteration_method = 'get_file_object')
    {
        ...
        if ($files === false || !is_string($iteration_method) || !method_exists($this, $iteration_method)) {
            return array();
        }
        return array_values(array_filter(array_map(
            function ($file_name) use ($iteration_method) {
                return $this->{$iteration_method}($file_name);
            },
            $files
        )));
    }
    ...
    protected function get_unique_filename(
        ...
        $uploaded_bytes = $content_range === null ? 0 : $this->fix_integer_overflow((int)$content_range[1]);
    ...
    protected function gd_create_scaled_image($file_name, $version, $options)
    {
        ...
        $type = strtolower(substr($extension, 1));
        switch ($type) {
            case 'jpg':
            case 'jpeg':
        ...
        // PHPStan GD stubs treat imagecopyresampled as always-true; keep runtime && for both operations.
    ...
    protected function download()
    {
        ...
        $modifiedAt = filemtime($file_path);
        if ($modifiedAt !== false) {
            $this->header('Last-Modified: ' . gmdate('D, d M Y H:i:s T', $modifiedAt));
        }
    ...
    public function generate_response($content, $print_response = true)
    {
        ...
        if ($print_response) {
            ...
            if ($this->get_server_var('HTTP_CONTENT_RANGE')) {
                ...
                if ($files && is_array($files) && is_object($files[0]) && property_exists($files[0], 'size') && $files[0]->size) {
                    $this->header('Range: 0-' . (
    ...
    public function post($print_response = true)
    {
        ...
        if ($content_range_header) {
            $parsed_content_range = preg_split('/[^0-9]+/', $content_range_header);
            if (is_array($parsed_content_range) && isset($parsed_content_range[1], $parsed_content_range[3])) {
        ...
        if ($content_range && is_file($this->get_upload_path($name))) {
    ...
    protected function basename($filepath, $suffix = '')
    {
        ...
        if ($splited === false) {
            return substr(basename('X' . rtrim($filepath, '/ '), $suffix), 1);
        }
