...
class BackendBlogCategoriesHelper
{
    ...
    {
        if (!empty($image['name']) && ($filename = $this->imageCore->uploadImage($image['tmp_name'], $image['name'], $this->config->get('original_blog_categories_dir')))) {
