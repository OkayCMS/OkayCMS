<?php

$lang['np_update_all'] = 'Update all cache now';
$lang['np_update'] = 'Update ';
$lang['np_warehouses_data_info'] = 'Warehouses data';
$lang['left_setting_np_title'] = 'New Mail';
$lang['settings_np'] = 'New Mail Settings';
$lang['settings_np_key'] = 'New Mail API Key';
$lang['settings_np_weight'] = 'Default weight (kg)';
$lang['settings_np_volume'] = 'Default capacity (м<sup style="font-size: 8px;">3</sup>)';
$lang['settings_np_city'] = 'City of dispatch';
$lang['settings_np_service_type'] = 'Delivery technology';
$lang['settings_np_service_dd'] = 'Doors-Doors';
$lang['settings_np_service_dw'] = 'Doors-Warehouse';
$lang['settings_np_service_ww'] = 'Warehouse-Warehouse';
$lang['settings_np_service_wd'] = 'Warehouse-Doors';
$lang['payment_method_np_cod'] = 'Cash on delivery for NP';
$lang['product_np_volume'] = 'Сapacity м<sup style="font-size: 8px;">3</sup>';
$lang['order_np_term'] = 'Delivery time (days)';
$lang['order_np_calc'] = 'Recalculate price and terms';
$lang['order_np_redelivery'] = 'C.O.D';
$lang['order_np_city'] = 'City';
$lang['order_np_street'] = 'Street';
$lang['order_np_house'] = 'Home';
$lang['order_np_apartment'] = 'Apartment';
$lang['order_np_warehouse'] = 'Warehouse';
$lang['order_np_area'] = 'Area';
$lang['payment_np_cash_on_delivery'] = 'Allow cash on delivery';
$lang['payment_np_payment_method_name'] = 'Payment method';
$lang['payment_np_cash_on_delivery_type'] = 'List of payment methods for cash on delivery';
$lang['okaycms__np__import_field'] = 'Variant volume';
$lang['settings_np_last_update_cities'] = 'Date of update of the cache of cities';
$lang['settings_np_last_update_warehouses'] = 'Date of update of the cache of points of issue';
$lang['np_update_cache_now'] = 'Refresh';
$lang['np_update_address_info'] = 'If you want to correctly update the Address field with the new data, please clear the Address field above before selecting a department.';
$lang['settings_np__description'] = 'The New Mail module allows you to select a city and branch in the basket and show the cost';
$lang['settings_np_options'] = 'Parameters';
$lang['tooltip_settings_np_api'] = 'You can get the API key in your account on the New Mail website';
$lang['tooltip_settings_np_weight'] = 'The weight of one product in the order, which will be used when calculating the cost of delivery. The maximum weight of the entire order can be no more than 1000 kg.';
$lang['tooltip_settings_np_volume'] = 'The volume of one product in the order, which will be used when calculating the cost of delivery';
$lang['tooltip_settings_np_city'] = 'The city from which orders will be sent from the online store';
$lang['settings_np_options_updating'] = 'Data update';
$lang['settings_np_update_date'] = 'Date of last update of data on points of issue:';
$lang['settings_np_update_label'] = 'To speed up the site, the data on the points of issue of New Mail are stored in the database of the site. <br> To update the data on points of delivery, click on the "Update" button.';
$lang['np_cron_update_cache_title'] = 'Advice';
$lang['np_cron_update_cache_1'] = 'In order for the data to be updated automatically, you should make sure that the server is configured to run the system task scheduler using cron:';
$lang['np_cron_update_cache_2'] = 'every minute (* * * * *) and the data will be updated automatically once a day.';
$lang['settings_np_include_volume'] = 'Include in the calculation of the cost of delivery the volume of cargo.';
$lang['settings_np_include_assessed'] = 'Include estimated cost in shipping costs';
$lang['np_update_type_cities'] = 'Cities';
$lang['np_update_type_cities'] = 'Cancel';
$lang['np_update_cache_finished'] = 'Finished';
$lang['np_api_key_error'] = 'API key error';
$lang['np_delete_delivery_type'] = 'Remove delivery type';
$lang['np_add_delivery_type'] = 'Add delivery type';
$lang['np_delivery_types_head'] = 'Delivery types';
$lang['np_warehouse_not_selected'] = 'Select warehouse';
$lang['np_error_city_id'] = 'Empty city_id parameter. Select the destination city from the dropdown list.';
