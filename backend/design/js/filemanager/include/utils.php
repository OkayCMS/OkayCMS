...
*/
function checkRelativePath(mixed $path): bool
{
    return normalizeFilemanagerRelativePath($path) !== null;
}

function normalizeFilemanagerRelativePath(mixed $path): ?string
{
    if (!is_scalar($path)) {
        return null;
    }
    ...
    if (
        ...
        || preg_match('/^[A-Za-z][A-Za-z0-9+.-]*:/', $decodedPath)
        || !checkRelativePathPartial($path)
        || !checkRelativePathPartial($decodedPath)
    ) {
        return null;
    }
    ...
    if ($normalized !== '' && str_ends_with($decodedPath, '/')) {
        $normalized .= '/';
    }
...
function normalizeFilemanagerUploadFolder(mixed $path): ?string
{
    if (!is_scalar($path)) {
        return null;
    }

    $normalized = normalizeFilemanagerRelativePath(trim(strip_tags((string)$path), '/'));
    if ($normalized === null) {
        return null;
    }
...
function sanitizeFilemanagerSvg(string $path): bool
{
    if (!is_file($path)) {
        return false;
    }
    ...
    if (!is_string($content) || trim($content) === '' || stripos($content, '<!DOCTYPE') !== false) {
        return false;
    }
    ...
    $loaded = $document->loadXML($content, LIBXML_NONET | LIBXML_COMPACT | LIBXML_NOERROR | LIBXML_NOWARNING);
    libxml_clear_errors();
    ...
    sanitizeFilemanagerSvgElement($root);
...
function sanitizeFilemanagerSvgElement(DOMElement $element): void
{
    $dangerousElements = array('script', 'foreignobject', 'iframe', 'object', 'embed', 'style', 'link');
    ...
    foreach ($element->childNodes as $childNode) {
        ...
        if (
            !in_array(strtolower((string)$childNode->localName), $dangerousElements, true)
            && $childNode->namespaceURI === 'http://www.w3.org/2000/svg'
        ) {
            sanitizeFilemanagerSvgElement($childNode);
            continue;
        }
    ...
    foreach ($element->attributes as $attribute) {
        ...
        $decodedValue = html_entity_decode($attribute->value, ENT_QUOTES | ENT_HTML5, 'UTF-8');

        if (
            str_starts_with($name, 'on')
            || $localName === 'style'
            || preg_match('/(?:javascript|data|vbscript|file):/i', $decodedValue)
            || (in_array($localName, array('href', 'src', 'action', 'formaction'), true)
                && trim($attribute->value) !== ''
                && !str_starts_with(trim($attribute->value), '#'))
        ) {
...
function resolveFilemanagerPath(mixed $basePath, mixed $relativePath): ?string
{
    if (!is_scalar($basePath)) {
        return null;
    }

    $normalized = normalizeFilemanagerRelativePath($relativePath);
    if ($normalized === null) {
        return null;
    }

    $baseRealPath = realpath((string)$basePath);
    if ($baseRealPath === false) {
        return null;
    }

    $baseRealPath = rtrim($baseRealPath, DIRECTORY_SEPARATOR);
    $candidate = $baseRealPath . ($normalized === '' ? '' : DIRECTORY_SEPARATOR . $normalized);
    $existingPath = realpath($candidate);
    $boundaryPath = $existingPath !== false ? $existingPath : realpath(dirname($candidate));

    if (
        $boundaryPath === false
        || ($boundaryPath !== $baseRealPath && !str_starts_with($boundaryPath, $baseRealPath . DIRECTORY_SEPARATOR))
    ) {
        return null;
    }
    ...
    if ($config['delete_files']) {
        ...
        if (!$ftp && $config['relative_image_creation']) {
            foreach ($config['relative_path_from_current_pos'] as $k => $path) {
                ...
                if (file_exists($dirname . "/" . $path . $config['relative_image_creation_name_to_prepend'][$k] . $info['filename'] . $config['relative_image_creation_name_to_append'][$k] . "." . $extension)) {
                    unlink($dirname . "/" . $path . $config['relative_image_creation_name_to_prepend'][$k] . $info['filename'] . $config['relative_image_creation_name_to_append'][$k] . "." . $extension);
                }
            }
        }
        ...
        if (!$ftp && $config['fixed_image_creation']) {
            foreach ($config['fixed_path_from_filemanager'] as $k => $path) {
                ...
                $base_dir = $path . substr_replace($dirname . "/", '', 0, strlen($config['current_path']));
                if (file_exists($base_dir . $config['fixed_image_creation_name_to_prepend'][$k] . $info['filename'] . $config['fixed_image_creation_to_append'][$k] . "." . $extension)) {
                    unlink($base_dir . $config['fixed_image_creation_name_to_prepend'][$k] . $info['filename'] . $config['fixed_image_creation_to_append'][$k] . "." . $extension);
                }
            }
        }
    }
...
function create_img($imgfile, $imgthumb, $newwidth, $newheight = null, $option = "crop", $config = array())
{
    ...
    if (isset($config['ftp_host']) && $config['ftp_host']) {
        if (url_exists($imgfile)) {
            ...
            $temp .= "." . ($extension === false ? '' : substr($extension, 1));
            $handle = fopen($temp, "w");
            $remoteImage = file_get_contents($imgfile);
            if ($handle === false || $remoteImage === false) {
                return false;
            }
            fwrite($handle, $remoteImage);
            fclose($handle);
            $imgfile = $temp;
            $save_ftp = $imgthumb;
            $imgthumb = $temp;
    ...
    if (file_exists($imgfile) || strpos($imgfile, 'http') === 0) {
        if (strpos($imgfile, 'http') === 0 || image_check_memory_usage($imgfile, $newwidth, (int)$newheight)) {
...
function create_folder($path = null, $path_thumbs = null, $ftp = null, $config = null)
{
    if ($ftp) {
        ...
        if ((is_string($path) && file_exists($path)) || (is_string($path_thumbs) && file_exists($path_thumbs))) {
            return false;
        }
        $oldumask = umask(0);
        $permission = 0755;
...
function fix_get_params($str)
{
    ...
    return strip_tags(is_string($str) ? $str : '');
}
...
function new_thumbnails_creation($targetPath, $targetFile, $name, $current_path, $config)
{
    ...
    if ($config['relative_image_creation']) {
        foreach ($config['relative_path_from_current_pos'] as $k => $path) {
            ...
            if (! ends_with($targetPath, $path)) {
                if (! create_img($targetFile, $targetPath . $path . $config['relative_image_creation_name_to_prepend'][ $k ] . $info['filename'] . $config['relative_image_creation_name_to_append'][ $k ] . "." . $extension, $config['relative_image_creation_width'][ $k ], $config['relative_image_creation_height'][ $k ], $config['relative_image_creation_option'][ $k ])) {
                    $all_ok = false;
                }
            }
        }
    ...
    if ($config['fixed_image_creation']) {
        foreach ($config['fixed_path_from_filemanager'] as $k => $path) {
            ...
            if (! create_img($targetFile, $base_dir . $config['fixed_image_creation_name_to_prepend'][ $k ] . $info['filename'] . $config['fixed_image_creation_to_append'][ $k ] . "." . $extension, $config['fixed_image_creation_width'][ $k ], $config['fixed_image_creation_height'][ $k ], $config['fixed_image_creation_option'][ $k ])) {
                $all_ok = false;
            }
        }
    }
...
function is_function_callable($name)
{
    ...
    $disabled = explode(',', $disabled_functions === false ? '' : $disabled_functions);
